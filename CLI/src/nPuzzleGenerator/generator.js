  ...
  new Promise((resolve) => {
    ...
    sizes.forEach((size) => {
      ...
      iterations.forEach((iter) => {
        ...
        exec(`./src/nPuzzleGenerator/puzzleGenerator.py ${size} -u -i ${iter} > ./resources/invalid_s${size}_i${iter}`);
        puzzlesGeneratedCounter += 1;

        // For each size and each iteration, create a valid puzzle
        exec(`./src/nPuzzleGenerator/puzzleGenerator.py ${size} -s -i ${iter} > ./resources/valid_s${size}_i${iter}`);
        puzzlesGeneratedCounter += 1;
