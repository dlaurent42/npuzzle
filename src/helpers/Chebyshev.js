...
// This class handles all data and methods relative to Chebyshev resolution
class Chebyshev extends Puzzle {
  ...
  getDistance(puzzle) {
    ...
    puzzle.forEach((el) => {
      ...
      distance += Math.max(Math.abs(snail.x - el.x), Math.abs(snail.y - el.y));
    });
    return distance;
  }
